...
<div class="chapter-tab-pane" data-tab="chapter-1">
  <div class="framework-chapter" data-chapter="1">
    ...
    <!-- 去练习测验按钮（必须在 framework-chapter 内部；公用页面位于 自考学习/ 根目录，同目录引用） -->
...
```json
{
  ...
  "type": "self-study",
  "renderMode": "content",
  "contentUrl": "自考学习/知识框架.html?subject=13003"
}
```

> 三科知识框架共用 `自考学习/知识框架.html` 单页（方案A-1），通过 `?subject=13015|02324|13003` 参数区分科目；各科掌握度数据存独立 localStorage 键（`ss_mastery_{科目代码}`）。
...
│   ├── NEW_AGENT_START_HERE.md           # 新 Agent 必读入口
│   ├── 用户偏好与AI协作建议.md             # 用户偏好档案 + 协作建议
│   ├── BACKLOG_TODO.md                   # 优化积木箱
│   ├── 交付检查清单.md                     # 提交前检查清单
│   ├── CSS抽离专项方案.md                  # CSS抽离落地执行细则
│   ├── CSS重复样式重构步骤清单.md           # CSS重构步骤清单
│   ├── SASS统一迁移完整方案.md             # SASS迁移完整方案
│   ├── SASS统一迁移方案.md                # SASS迁移方案（早期版本）
│   ├── 公共JS与CSS抽离方案B.md             # 公共JS与CSS抽离总方案
│   ├── 离散数学学习闭环标准模板.md           # 学习闭环标准模板
│   ├── 数据结构与算法-卡片与题目设计规范.md   # 数据结构设计规范
│   ├── 离散数学-卡片与题目设计规范.md        # 离散数学设计规范
│   ├── 计算机系统原理-卡片与题目设计规范.md   # 系统原理设计规范
│   └── 新增知识点提示词模板.md             # 新增知识点AI提示词模板
│
...
│   │   ├── 备考科目/
│   │   │   ├── 13015计算机系统原理/         # PDF教材 + 历史备份
│   │   │   ├── 02324离散数学/               # PDF教材 + 历史备份
│   │   │   └── 13003数据结构与算法/         # PDF教材
│   │   └── 未考科目/
