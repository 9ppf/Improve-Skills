...
**核心变更**：补全 13015 计算机系统原理第4-6章考核知识点与考核要求（来自教材 PDF 第17-21页官方大纲）；补充第4-6章题库 65 题，实现 84 个核心概念全覆盖。

| 文件 | 更新内容 |
|---|---|
| `data/knowledge-framework-13015.json` | 第4章新增5条考核主题、第5章新增6条考核主题、第6章新增4条考核主题（含识记/领会/简单应用/综合应用层级） |
| `data/quiz-bank-13015.json` | 新增 65 题（第4章20题/第5章25题/第6章20题），总题数 405→470，84个核心概念覆盖率 100% |

**第4章 可执行文件的生成与加载**（+20题）：编译链接四阶段、ELF文件格式（节/段/程序头表）、符号解析与重定位、静态/动态链接、异常与中断、指令流水线、加载器

**第5章 程序的存储访问**（+25题）：存储器层次结构、局部性原理、cache映射方式与地址划分、cache平均访问时间计算、写策略与替换算法、TLB快表、虚拟存储与页表、多级页表

**第6章 程序中I/O操作的实现**（+20题）：I/O子系统层次、三种I/O控制方式比较、中断处理过程、DMA工作原理、磁盘调度算法（SSTF/SCAN）、总线结构与带宽计算、通道控制方式
