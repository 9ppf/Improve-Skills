...
| v2.18.3 | 2026-08-20 | 背诵与简答模块计算卡优化(cardType=calculation+公式/步骤/答案三段式+answerAliases等价答案匹配+符号面板+答对答错自动更新掌握度) + 三科各新增3张计算卡示例 |
| v2.18.4 | 2026-08-21 | 练习测验模块开发(独立页面/按科目分离?subject=/6种题型差异化/统一评分调度/4种出题模式) + 三科独立题库(各16题) + quiz-bank API |
| v2.19.0 | 2026-08-21 | 学习闭环整合(背诵卡纯粹化+练习测验升级+复盘总结升级) + 引导流按钮+URL传参 + AI批量出题+弱点优先 + 洞察条+错题归集 + quiz-records API + 排版优化(练习测验5层→3层/复盘总结紧凑头部+分段标签+内联洞察条) + 删除科目下拉选择器 |
| v2.20.0 | 2026-08-22 | 章节精讲样式1:1还原+三科统一 + 约束体系Skill化：章节精讲样式还原（筛选药丸/可折叠分类卡片/知识点颜色标签/底部测验按钮），从13015同步到02324和13003；启动Skill化约束建设，新增3个Skill（structural-change-workflow结构性变更 / documentation-versioning文档版本 / ui-design-restoration设计稿还原）+ 5个配套校验脚本 + 更新用户偏好与协作建议 |
| v2.19.3 | 2026-08-21 | 知识框架结构级1:1还原：整体定位→简洁卡片、章节卡片→class+彩色边框+状态图标、完整目录→可折叠卡片、C程序旅程→步骤卡片，新增kf-card/journey-step/collapse-card等组件 |
| v2.19.2 | 2026-08-21 | 知识框架样式1:1还原设计稿：header/section/chapter-grid/ch-card等CSS细节对齐，accent主色统一#3b82f6，gap/padding/margin等微调到设计稿值 |
...
> **当前版本：v2.20.0** — 详见 `CHANGELOG.md` v2.20.0 章节。

---

## 项目一句话描述

一个 Python 构建的静态 HTML 个人工作台，聚合「今日学习」「能力提升」「自考学习」「Python 基础」「AI 学习」「AI 助手角色」与「阅读资料」七大模块，主题可切换，支持本地热重载预览。v2.19.3 当前状态：侧边栏采用字号+字重分层（无图标/无进度条），支持flat分类跳过冗余中间层，自考学习模块已形成学习闭环（背诵卡→练习测验→复盘总结→间隔复习），三科独立题库+答题记录持久化，练习测验支持6种题型+4种出题模式（含AI批量出题和弱点优先），复盘总结支持错题反思+题库管理+洞察条，知识框架页结构级1:1对齐设计稿（整体定位卡片+章节彩色边框+可折叠目录+步骤旅程），掌握状态和AI对话通过API实现跨浏览器持久化。
...
> 以下来自 v2.19.x-v2.20.0 设计稿还原任务的血泪教训，反复踩坑后固化。

27. **还原模式默认严格** — 有设计稿/有明确参照的任务，默认按1:1精确还原执行。用户没说"大概就行"就默认精确，不能"差不多"。
28. **改之前先列差异清单** — 涉及设计稿还原/多处样式改动，先列表格（位置/当前值/目标值/改/不改），用户确认后再动手。边想边改=反复返工。
29. **验证必须走工作台路径** — 必须从侧边栏点击对应模块，通过iframe查看效果。直接打开HTML文件 = 没验证，因为可能有缓存、样式冲突、路径问题。
30. **同一个问题改2次还不对，立即求助** — 说明理解有偏差，再试也是浪费时间。停下来说清：我理解的需求、我做了什么、结果是什么、我卡在哪。
31. **两种模式：还原 vs 创作** — 有明确目标或参照=还原模式（严格执行不发挥）；无明确目标=创作模式（可以主动给方案2-3个）。判断标准：用户有没有给出明确的目标或参照。
32. **知识点优先级必须颜色区分** — 不能只用文字标签（🔴重点/🟡一般/🟢了解），必须用字体颜色或背景色区分，让用户一眼抓住重点。背景色区分 > 字体颜色区分 > 纯标签。
33. **用户没说改的一律不动** — "顺手优化"、"这样更好看"是大忌。说改A就只改A，B、C、D哪怕有问题也先不说，除非用户问。
34. **交付必须问"可以提交吗？"** — 不能只说"做完了"，必须明确征求验收同意，等用户确认后再提交Git。
...
> 以下为截至 v2.20.0 的待办优先级，供新 Agent 参考。

### 高优先级

1. **约束体系落地验证** — 3个新Skill（结构性变更/文档版本/设计稿还原）已创建，配套校验脚本已写，但还需要在实际任务中验证是否能有效卡点。下一次涉及设计稿还原或结构性变更时，主动调用对应Skill，检查校验脚本是否能发现问题。
2. **三科章节精讲内容补齐** — v2.20.0 只完成了样式还原和三科同步，章节精讲的实际内容（知识点详解、例题等）还需要从教材中逐步补齐。
3. **真题内容填充** — 三科真题与错题本目前框架完整但内容不足，需要从近5年真题中录入高频考点。

### 中优先级

4. **Skill化约束持续建设** — 目前只有3个Skill，还有多类约束未Skill化（见`文件约束隐患与规避方案.md`第三章未完成清单），按优先级逐步推进：知识点优先级标注校验 > 缓存控制校验 > temp目录规范 > 实现即测试。
5. **移动端响应式适配** — 已支持手机局域网访问（`--host 0.0.0.0`），但大部分页面无响应式布局，侧边栏/卡片在手机上体验不佳。
6. **间隔重复调度** — 英语词汇已有SM-2算法，自考三科背诵卡可复用同一算法安排复习频率，让"不会"的卡片间隔重现。

### 低优先级

7. **清理临时脚本** — `temp/` 下有多个临时脚本，已加入.gitignore，但可考虑整理或删除。
8. **其他localStorage数据迁移** — Python/AI知识点掌握状态、英语进度等仍在localStorage，如需跨浏览器保留可参照v2.17.0方案迁移。
9. **Python知识点手写练习验证** — 29个知识点的手写练习代码尚未在Python环境中实际运行验证。
10. **AI Demo代码实现** — 4个Demo目前为设计文档+伪代码，后续可在实际Python环境中创建项目并实现。
