...
> 把本文档直接交给新 Agent，并告诉他当前任务即可开始工作。文档末尾「v2.1.0 后续建议」列出待办优先级。

> **⚠ 重要：v2.0.0 和 v2.1.0 变更已提交 Git**（commit `2975402`）。工作区仅剩 AI News Digest 定时任务产物未跟踪。**禁止执行 `git reset --hard`、`git checkout .`、`git clean -f` 等破坏性操作**，以免丢失未跟踪的工作。
...
> **当前版本：v2.1.0** — 详见下方「v2.1.0 更新内容」章节。

---

## 项目一句话描述

一个 Python 构建的静态 HTML 个人工作台，聚合「能力提升」「自考学习」「Python 基础」「AI 学习」「AI 助手角色」与「阅读资料」六大模块，主题可切换，支持本地热重载预览。v2.1.0 新增：背诵卡测验模式（AI 自动出题 + 答题反馈 + 掌握度联动）；dev_server.py 支持手机局域网访问。
