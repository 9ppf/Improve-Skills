...
> 把本文档直接交给新 Agent，并告诉他当前任务即可开始工作。文档末尾「v2.4.0 后续建议」列出待办优先级。

> **⚠ 重要：v2.0.0 至 v2.2.0 变更已提交 Git**（commit `80d9c5a`）。v2.3.0 至 v2.4.0 变更待提交。工作区仅剩 AI News Digest 定时任务产物和 study-plan-optimization 临时文件未跟踪。**禁止执行 `git reset --hard`、`git checkout .`、`git clean -f` 等破坏性操作**，以免丢失未跟踪的工作。
...
> **当前版本：v2.4.0** — 详见下方「v2.4.0 更新内容」章节。

---

## 项目一句话描述

一个 Python 构建的静态 HTML 个人工作台，聚合「能力提升」「自考学习」「Python 基础」「AI 学习」「AI 助手角色」与「阅读资料」六大模块，主题可切换，支持本地热重载预览。v2.4.0 新增：工作台风格统一（蓝色主题+renderMode 三种渲染模式）；Python 模块四项功能优化（答案折叠+代码复制+真实数据+双向联动）。
...
3. **Demo 代码实测** — 4个Demo的完整代码已内嵌页面，但尚未在实际 Python 环境中创建项目并运行测试。
4. **移动端适配** — 已支持手机访问但无响应式适配。
5. **AI 学习板块重设计** — 用户计划重新设计 AI 学习模块（下一步讨论）。
