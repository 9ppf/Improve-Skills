...
> **⚠ 重要：v2.0.0 至 v2.4.0 变更已提交 Git**（commit `af398ff`）。工作区仅剩 AI News Digest 定时任务产物和 study-plan-optimization 临时文件未跟踪。**禁止执行 `git reset --hard`、`git checkout .`、`git clean -f` 等破坏性操作**，以免丢失未跟踪的工作。
