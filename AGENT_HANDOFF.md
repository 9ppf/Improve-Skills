...
- **技术栈**：Python 构建的静态 HTML，localStorage + JSON 文件持久化
...
1. **三科知识框架 JSON 化** — 三个独立 HTML 文件从同一模板复制后手动修改内容，导致结构不一致。方案：一个模板 HTML + 三科 JSON 数据文件，JS 动态渲染。
...
6. **Skill化约束持续建设** — 还有知识点优先级标注校验 > 缓存控制校验 > temp目录规范 > 实现即测试。
7. **移动端响应式适配** — 已支持手机局域网访问（`--host 0.0.0.0`），但大部分页面无响应式布局。
8. **间隔重复调度** — 英语词汇已有SM-2算法，自考三科背诵卡可复用同一算法。
9. **练习测验题库扩充** — 三科目前各只有 16 题，需要补充。

#### 低优先级

10. **清理临时脚本** — `temp/` 下有多个临时脚本，可考虑整理或删除。
11. **其他localStorage数据迁移** — Python/AI知识点掌握状态、英语进度等仍在localStorage。
12. **Python知识点手写练习验证** — 29个知识点的手写练习代码尚未在Python环境中实际运行验证。
13. **AI Demo代码实现** — 4个Demo目前为设计文档+伪代码。
